...
class TestOffer(APITestCase):
    ...
    def test_offer_list_filter_by_creator_id_get_200(self):
        ...
        response = self.user_client_1.get(url)
        expected_dict = {"count": 0, "next": None, "previous": None, "results": []}
        self.assertDictEqual(response.json(), expected_dict)
    ...
    def test_offer_list_filter_by_max_delivery_time_get_200(self):
        ...
        expected_dict = {
            ...
            "results": [
                OfferListSerializer(
                    self.offer_1, context={"request": response.wsgi_request}
                ).data,
            ],
        }
        self.assertDictEqual(response.json(), expected_dict)
    ...
    def test_offer_list_search_title_get_200(self):
        ...
        expected_dict = {
            ...
            "results": [
                OfferListSerializer(
                    self.offer_3, context={"request": response.wsgi_request}
                ).data
            ],
        }
        self.assertDictEqual(response.json(), expected_dict)
    ...
    def test_offer_post_400(self):
        ...
        for field in required_fields:
            ...
            payload_missing_field["details"] = [
                d.copy() for d in base_payload["details"]
            ]
            payload_missing_field["details"][0].pop(field)
            response = self.user_client_1.post(
                url, payload_missing_field, format="json"
            )
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
